...
while (1):
    ...
    try:  # an error comes if it does not find anything in window as it cannot find contour of max area
        ...
        if cv2.waitKey(5) & 0xFF== ord('b'):
           roi = removeBG(roi)
        #make_frame = cut roi to (x :80~120, y: 90~130)
        make_frame = roi[90:130,80:120]
        #hhhsv = make_frame to hsv
        hhhsv  = cv2.cvtColor(make_frame,cv2.COLOR_BGR2YCrCb)
        ...
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2YCrCb)

        # define range of skin color in HSV
        lower_skin = np.array([h-20, s-10, v-10], dtype=np.uint8)
        upper_skin = np.array([h+20, s+10, v+10], dtype=np.uint8)
